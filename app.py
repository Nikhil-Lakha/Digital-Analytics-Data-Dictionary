...
def require_admin(prefix):
    if not get_token() and not get_admin_password(): return True
    if st.session_state.get("admin_authenticated", False): return True
    password = get_admin_password()
    if not password:
        st.warning("Editing is locked until ADMIN_PASSWORD is added to Streamlit Secrets."); return False
...
def get_data(): return load_dictionary(get_token() or None)
...
def dropdown(frame, field, current, key):
    options = list(DROPDOWN_OPTIONS[field])
    # Preserve any existing legacy/custom value so editing never destroys it.
    current_text = str(current).strip()
    if current_text and current_text not in options:
        options.append(current_text)
    if not current_text:
        defaults = {"Status":"Draft", "Business Criticality":"Medium", "Required":"No", "Send to AWS":"Yes", "Contains PII":"No", "Journey":"Global / All Journeys", "Subcategory":"Not Applicable"}
        current_text = defaults.get(field, options[0])
    return st.selectbox(field + (" *" if field in {"Category", "Data Type"} else ""), options, index=options.index(current_text) if current_text in options else 0, key=key)


def build_variable_form(frame, row=None, prefix="form"):
    values, current = {}, ({} if row is None else row.to_dict())
    ...
    with c1:
        ...
        values["Category"] = dropdown(frame, "Category", current.get("Category", ""), f"{prefix}_category")
    with c2:
        values["Data Type"] = dropdown(frame, "Data Type", current.get("Data Type", ""), f"{prefix}_data_type")
    ...
    rendered = {"Variable Name", "Friendly Name", "Category", "Data Type", "Tealium Variable Name", "AWS Field Name"}
    text_areas = {"Definition", "Allowed Values", "Notes"}
    cols = st.columns(2)
    for idx, field in enumerate([c for c in frame.columns if c not in rendered]):
        default = current.get(field, "")
        if row is None:
            if field == "Schema Version": default = "1.0"
            elif field in {"Date Added", "Last Updated"}: default = date.today().isoformat()
        with cols[idx % 2]:
            if field in DROPDOWN_OPTIONS:
                values[field] = dropdown(frame, field, default, f"{prefix}_{field}")
            elif field in text_areas:
...
try: df = get_data()
except Exception as exc: st.error(f"Could not load the analytics dictionary: {exc}"); st.stop()


@st.dialog("Add new variable")
def add_variable_dialog():
    st.caption("Create a new record in the Digital Analytics Data Dictionary.")
    if not require_admin("add"): return
    if not get_token(): st.info("Local test mode: Save will update your local Excel workbook only.")
    ...
    if submitted:
        ...
        if missing: st.error(f"Please complete: {', '.join(missing)}"); return
        try: create_variable(get_token() or None, values); st.cache_data.clear(); st.rerun()
        except Exception as exc: st.error(f"Could not add the variable: {exc}")


@st.dialog("Variable information")
def information_dialog(variable_name):
    row = find_row(df, variable_name)
    if row is None: st.error("That variable could not be found."); return
    st.markdown(f"## {esc(row.get('Friendly Name', variable_name))}", unsafe_allow_html=True)
    st.caption(str(row.get("Variable Name", "")))
    main = {"Variable Name", "Friendly Name", "Category", "Data Type", "Tealium Variable Name", "AWS Field Name", "Tealium Variable Type", "Send to AWS"}
    d1, d2 = st.columns(2)
...
def edit_variable_dialog(variable_name):
    row = find_row(df, variable_name)
    if row is None: st.error("That variable could not be found."); return
    if not require_admin(f"edit_{variable_name}"): return
    with st.form(f"edit_{variable_name}"):
        values = build_variable_form(df, row, f"edit_{variable_name}")
        submitted = st.form_submit_button("Save changes", type="primary", use_container_width=True)
    if submitted:
        try: update_variable(get_token() or None, variable_name, values); st.cache_data.clear(); st.rerun()
        except Exception as exc: st.error(f"Could not save changes: {exc}")
...
def delete_variable_dialog(variable_name):
    ...
    if not require_admin(f"delete_{variable_name}"): return
    ...
    if st.button("Delete variable", type="primary", disabled=not confirm, use_container_width=True):
        try: delete_variable(get_token() or None, variable_name); st.cache_data.clear(); st.rerun()
        except Exception as exc: st.error(f"Could not delete variable: {exc}")
...
with f1: category = st.multiselect("Category", unique_values(df,"Category"))
with f2: data_type = st.multiselect("Data type", unique_values(df,"Data Type"))
with f3: status = st.multiselect("Status", unique_values(df,"Status"))
with f4: owner = st.multiselect("Owner", unique_values(df,"Owner"))
filtered = df.copy()
if search.strip():
    term=search.strip().lower(); mask=pd.Series(False,index=filtered.index)
    for column in ["Variable Name","Friendly Name","Definition"]: mask |= filtered[column].fillna("").astype(str).str.lower().str.contains(term,regex=False)
...
with heading_col: st.markdown(f"### Variables ({len(filtered)})")
with add_col:
    if st.button("＋ Add new variable",type="primary",use_container_width=True): add_variable_dialog()
...
for col,label in zip(st.columns(widths),headers):
    with col: st.markdown(f'<div class="table-header">{label}</div>',unsafe_allow_html=True)
st.markdown('<div class="row-divider"></div>',unsafe_allow_html=True)
if filtered.empty: st.info("No variables match the current filters.")
else:
    for idx,row in filtered.reset_index(drop=True).iterrows():
        variable_name=str(row.get("Variable Name","")); cols=st.columns(widths)
        for col,value in zip(cols[:5],[f"**{variable_name}**",str(row.get("Friendly Name","")),str(row.get("Category","")),str(row.get("Tealium Variable Name","")),str(row.get("AWS Field Name",""))]):
            with col: st.markdown(value)
        with cols[5]:
            if st.button("Info",key=f"info_{idx}_{variable_name}",use_container_width=True): information_dialog(variable_name)
        with cols[6]:
            if st.button("Edit",key=f"edit_{idx}_{variable_name}",use_container_width=True): edit_variable_dialog(variable_name)
        with cols[7]:
            if st.button("Delete",key=f"delete_{idx}_{variable_name}",use_container_width=True): delete_variable_dialog(variable_name)
        st.markdown('<div class="row-divider"></div>',unsafe_allow_html=True)
